<script>
  ...
  let griddedDataset = hash.gdata ?? gDatasets.find(d => d.name === 'temperature at 2 m above ground') ?? gDatasets[0];
  let displayedGriddedDataset = griddedDataset;
  let particleDataset = hash.pdata ?? pDatasets.find(d => d.name === 'wind at 10 m above ground') ?? pDatasets[0];
  ...
  let date = hash.date ?? ((__production__ || !__using_local_data_files__) ? griddedDataset.closestValidDate($currentDate) : griddedDataset.end);
  let anchorDate = date;
  let displayedDate = date;

  let projection = projections.EQUIRECTANGULAR;
  
  $: forwardProjectionFunction = projection.function;
  $: inverseProjectionFunction = projection.function.invert;
  let canvasRatio = 1;
  ...
  function getTimeDataset(griddedDataset, particleDataset) {
    return griddedDataset === GriddedDataset.none && particleDataset !== particleDataset.none ? particleDataset : griddedDataset;
  }

  function fixDate(timeDataset) {
    let valid = timeDataset.closestValidDate(anchorDate);
    if (date.getTime() !== valid.getTime()) date = valid;
  }

  function hideParticlesIfTooFar(date) {
    if (!particleDataset.isCloseDate(date)) particleDataset = ParticleDataset.none;
  }
  ...
  async function updateDataAndVariables(griddedDataset, particleDataset, date) {
    ...
    if (displayedDate.getTime() !== date.getTime()) displayedDate = date;
    if (displayedTimeDataset !== timeDataset) displayedTimeDataset = timeDataset;
  }

  async function getData(dataset, date, signal) {
    try { return await dataset.fetchData(date, signal); } 
    catch(e) { if (e.name === 'AbortError') return false; console.error(e); return dataset.emptyData; }
  }

  griddedDataset.fetchData(date, controller.signal).then(data => griddedData = data);
  particleDataset.fetchData(date, controller.signal).then(data => particleData = data);
  ...
  function fadeSplashScreen() {
    ...
    setTimeout(() => splashElement.remove(), 1000 * parseFloat(getComputedStyle(splashElement)['transitionDuration']));
  }

  // Force a window resize event to tell the Map component to redraw
  onMount(() => {
...
<div class="wrapper">
  <Hash
    bind:date bind:griddedDataset bind:particleDataset bind:projection
    bind:centerLatitude bind:centerLongitude bind:zoom bind:simplifiedMode
    bind:kioskMode bind:pins {gDatasets} {pDatasets} {minZoom} {maxZoom}
  />
  <Navbar {menus} bind:openedMenu bind:drawerOpen />
  <Drawer {menus} bind:openedMenu bind:drawerOpen {kioskMode} />
  
  <Menu bind:openedMenu menuName="Help & About"><About {kioskMode} /></Menu>
  <Menu bind:openedMenu menuName="Datasets" bind:simplifiedMode>
    <Datasets {date} {gDatasets} {pDatasets} bind:griddedDataset bind:particleDataset {simplifiedMode} />
  </Menu>
  <Menu bind:openedMenu menuName="Projections">
    <Projections bind:projection bind:centerLongitude bind:centerLatitude bind:zoom />
  </Menu>
  
  {#if !kioskMode}
    <Menu bind:openedMenu menuName="Markers">
      <Locations bind:centerLongitude bind:centerLatitude bind:zoom bind:pins {griddedData} {griddedUnit} />
    </Menu>
    <Menu bind:openedMenu menuName="Feedback"><Feedback /></Menu>
  {/if}

  <main>
    <div class="map-viewport">
      <Map
        {projection} {centerLongitude} {centerLatitude} {zoom} {portraitBasedZoom}
        {griddedData} {griddedColormap} {griddedDomain} {griddedScale}
        {particleData} {particlesPaused} {particleLifetime} {particleCount} {particleDisplay}
        {vectorData} {vectorColors}
      ...
      >
        ...
        <Surface
          {minZoom} {maxZoom} bind:centerLongitude bind:centerLatitude bind:zoom
          {portraitBasedZoom} {canvasRatio} {inverseProjectionFunction}
          bind:pins bind:cursor {kioskMode}
        />
        <Queries
          bind:pins {cursor} {forwardProjectionFunction} {griddedName}
          {griddedData} {griddedUnit} {griddedDomain} {griddedScale}
          {griddedColormap} {kioskMode}
        />
        <Loading />
        <Widgets
          bind:openedMenu {displayedDate} {displayedTimeDataset} bind:utc
          bind:centerLatitude bind:centerLongitude bind:zoom {minZoom} {maxZoom}
          {griddedName} {griddedColormap} {griddedDomain} {griddedScale} {griddedData}
          bind:griddedUnit {particleName} {particleDisplay} bind:particlesPaused
          {simplifiedMode} {kioskMode}
        />
      </Map>
    </div>
...
<style>
  ...
  main {
    ...
    height: 100vh; /* Critical for flex to work */
    width: 100%;
    background: #000;
  }
  ...
  .map-viewport {
    ...
    position: relative;
    width: 100%;
    /* These force the children (the map) to fill the remaining area */
    display: flex;
    flex-direction: column;
  }

  /* Extremely direct CSS to force the Map component to appear */
  :global(.map-viewport > *) {
